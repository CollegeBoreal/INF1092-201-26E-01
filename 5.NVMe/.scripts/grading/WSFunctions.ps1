...
function Get-ParticipationGrades {
    ...
    foreach ($line in $lines) {
        ...
        if ($line -match '^\|\s*\d+\s*\|') {
            ...
            $levels = @(474, 475, 476)  # fail, silver, gold
            ...
            $appLevels = @(479, 480, 481, 482)  # fail, bronze, silver, gold
...
function New-LMSRubricFromEntry {
    ...
    # Build rubric
    $rubric = @(
        @{ criterionid = 202;  levelid = $Entry.readme;    remark = "Quantité README.md " }
        @{ criterionid = 203;  levelid = $Entry.image;     remark = "Présence répertoire images " }
        @{ criterionid = 204;  levelid = $Entry.app;       remark = $Entry.comments }
    )
